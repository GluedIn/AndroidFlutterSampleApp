package com.gluedin.sample.payment
...
object BillingManager {
    ...
    private fun consumeExistingPurchases(onComplete: () -> Unit) {
    ...
    private fun launchPurchaseFlow(activity: Activity) {
        ...
        consumeExistingPurchases {
            ...
            // Now safe to launch billing flow
            launchNewPurchase(activity)
        }
    }
    private fun launchNewPurchase(activity: Activity) {

        // ✅ Validate SKU
        if (mSkuId.isNullOrEmpty()) {
            paymentCallback(PaymentStatus.PaymentFailed, "")
            return
        }
        ...
        val productList = listOf(
            QueryProductDetailsParams.Product.newBuilder()
                .setProductId(mSkuId!!)
                .setProductType(BillingClient.ProductType.INAPP) // ✅ Important
                .build()
        )
        ...
        mBillingClient?.queryProductDetailsAsync(params) { billingResult, queryResult ->
            ...
            // ✅ INAPP does NOT need offer token
            val productParams =
            ...
            val billingFlowParams = BillingFlowParams.newBuilder()
                ...
                .setObfuscatedAccountId(userId.orEmpty())
                .build()

            val flowResult = mBillingClient?.launchBillingFlow(activity, billingFlowParams)
    ...
    private fun handlePurchase(purchase: Purchase) {
        ...
        when (purchase.purchaseState) {
            ...
            Purchase.PurchaseState.PURCHASED -> {
                ...
                val orderId = JSONObject(purchase.originalJson)
                    .optString("orderId", "")
                ...
                if (mProductType == BillingClient.ProductType.INAPP) {
                    ...
                    mBillingClient?.consumeAsync(consumeParams) { billingResult, _ ->
                        ...
                        if (billingResult.responseCode != BillingClient.BillingResponseCode.OK) {
                            Log.e("Billing", "Consume failed but purchase already successful")
                            // ❌ DO NOT send PaymentFailed here
                        }
                    }
                ...
                if (!purchase.isAcknowledged) {
                    ...
                    mBillingClient?.acknowledgePurchase(ackParams) { br ->
                        ...
                        if (br.responseCode == BillingClient.BillingResponseCode.OK) {
                            ...
                            Log.e("Billing", "Acknowledge failed but purchase successful")
                            // ❌ DO NOT send failure
                        }
                    }
...
//    private fun extractPricePartsFromProductDetails(pd: ProductDetails?): PriceParts? {
...
//                currencySymbol = safeCurrencySymbol(it.priceCurrencyCode),
...
//
//        // ---------- SUBS ----------
//        val offer = pd.subscriptionOfferDetails
//            ?.firstOrNull { it.pricingPhases.pricingPhaseList.isNotEmpty() }
//            ?: return null
//
//        val phase = offer.pricingPhases.pricingPhaseList
...
//
//        return PriceParts(
//            amount = phase.priceAmountMicros / 1_000_000.0,
//            currencySymbol = safeCurrencySymbol(phase.priceCurrencyCode),
//            currencyCode = phase.priceCurrencyCode,
//            productId = pd.productId,
//        )
    ...
    fun openPlayStoreSubscription(
        ...
        try {
            context.startActivity(
                Intent(Intent.ACTION_VIEW, Uri.parse(subscriptionUrl)).setPackage(context.packageName)
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            )
        } catch (e: Exception) {
            // Fallback to browser
            context.startActivity(
                Intent(Intent.ACTION_VIEW, Uri.parse(fallbackUrl)).setPackage(context.packageName)
                    .addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            )
        }
    }
